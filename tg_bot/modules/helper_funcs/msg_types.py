...
def get_note_type(msg: Message):
    ...
    if len(args) >= 3:
        ...
        entities = (
            msg.reply_to_message.parse_entities()
            or msg.reply_to_message.parse_caption_entities()
        )
...
def get_welcome_type(msg: Message):
    ...
    if len(args) >= 2 and not msg.reply_to_message:
        ...
        content = msg.reply_to_message.video_note.file_id
        text, buttons = button_markdown_parser(
            msgtext, entities=msg.reply_to_message.parse_caption_entities(), offset=0
        )
        data_type = Types.VIDEO_NOTE
