...
from tg_bot import LOGGER, SUDO_USERS, dispatcher
    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 1:
            ...
            if var == "no":
                ...
                update.effective_message.reply_text(
                    "Please enter on/yes/off/no in group!"
                )
        else:
    ...
    if update.effective_chat.type == "private":
        if len(args) >= 1:
            ...
            if (
                ...
                or (sql.allow_connect_to_chat(connect_chat) == True)
                ...
                update.effective_message.reply_text(
                    "Connections to this chat not allowed!"
                )
        else:
...
CONNECT_CHAT_HANDLER = CommandHandler(
    "connect", connect_chat, allow_edited=True, pass_args=True
)
